...
def success():
    yes_button_pause = Button((WIDTH // 2 - 90, HEIGHT // 2 - 30), yes_button_img)
    no_button_pause = Button((WIDTH // 2 + 90, HEIGHT // 2 - 30), no_button_img)
    button_list = pygame.sprite.Group()
    button_list.add(yes_button_pause, no_button_pause)
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if yes_button_pause.rect.collidepoint(mouse_pos):
                    return True
                if no_button_pause.rect.collidepoint(mouse_pos):
                    return False
